...
function init() {
    ...
    if (checkForMathML && (msg = checkMathML()))
        warnings.push(msg);
    if (checkIfSVGavailable && (msg = checkSVG()))
        warnings.push(msg);
...
function setText(st, id) { // add text to an existing node with given id
    ...
    if (node != null)
        if (node.childNodes.length != 0)
            node.childNodes[0].nodeValue = st;
        else
...
//alert(src)
    ...
    try {
        with (Math)
            eval(src);          // here the svgpicture object is created
    } catch (err) {
        if (err != "wait") {
//alert(dsvglocation)
...
function drawPictures() { // main routine; called after webpage has loaded
    ...
    for (index = lastSlot; index < len + lastSlot; index++) {
        ...
        if (!/axes\b|initPicture/.test(src)) {
            var i = 0;
            while (/((yscl|ymax|ymin|xscl|xmax|xmin|\bwidth|\bheight)\s*=\s*-?\d*(\d\.|\.\d|\d)\d*\s*;?)/.test(src.slice(i)))
                i++;
...
function switchTo(id) { // used by dynamic code to select appropriate graph
    ...
    if (typeof name != "string")
        name = id.target.parentNode.getAttribute("name");
...
function updatePicture(obj) {
    ...
    if (typeof obj == "object")
        ...
        id = (typeof obj == "string" ? obj : "picture" + (obj + 1));
...
function mClick(evt) {
    if (sinceFirstClick != 0) {
        if (sinceFirstClick <= 40) {
            if (evt.shiftKey) {
                if (evt.altKey)
                    changepicturesize(evt, 2);
                else
                    zoom(evt, .5);
            } else if (evt.altKey)
                zoom(evt, 2);//changepicturesize(evt,.5);
            else
                showHideCode(evt);             // do this on dblclick
            clearTimeout(dblClkTimer);
            dblClkTimer = "";
        } else {
            clearTimeout(dblClkTimer);
...
function line(p, q, id, endpts) { // segment connecting points p,q (coordinates in units)
    ...
    if (marker == "dot" || marker == "arrowdot") {
        ...
        arrowhead(p, q);
    if (endpts == null && endpoints != "")
        endpts = endpoints;
    if (endpts != null) {
        if (endpts.indexOf("<-") != -1)
            arrowhead(q, p);
        if (endpts.indexOf("o-") != -1)
            dot(p, "open");
        if (endpts.indexOf("*-") != -1)
            dot(p, "closed");
        if (endpts.indexOf("->") != -1)
...
function path(plist, id, c, endpts) {
    ...
    if (endpts != null) {
        ...
        if (endpts.indexOf("-o") != -1)
            dot(plist[plist.length - 1], "open");
        if (endpts.indexOf("-*") != -1)
            dot(plist[plist.length - 1], "closed");
    }
}
...
//alert(dnode.childNodes.length)
    ...
    if (fontstroke != "none")
        node.setAttribute("stroke", fontstroke);
    if (fontfill != "none")
        node.setAttribute("fill", fontfill);
    return p;
}
...
function initASCIIMathCalculators(li) {
    ...
    for (i = 0; i < li.length; i++) {
        ...
        if (st != null)
            eval(String(st).replace(/function anonymous\(\)/, ""));
    }
}
...
function calculate(inputId, outputId) {
    ...
    if (!isNaN(res) && res != "Infinity")
        ...
        str = "`" + str + "` = undefined"; //debug:+mathjs(str);
...
function generic()
{
    ...
    if (translateOnLoad) {
        ...
        if (nd != null)
            dsvglocation = nd.className;
        if (nd != null || !checkforprocessasciimathinmoodle) {
            translate();
            if (!noSVG && translateASCIIsvg)
                drawPictures();
        }
        var li = getElementsByClass(document, "div", "ASCIIMathCalculator");
        if (!noMathML && li.length > 0)
            initASCIIMathCalculators(li);
    }
}
;
